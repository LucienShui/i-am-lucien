...
<template>
  <div id="chat">
    ...
    <form id="chat-form" @submit.prevent="submit">
      ...
      <input id="chat-text" ref="chat-text" class="chat-input" spellcheck="false" v-model="text" onblur="this.focus()"
             autofocus>
    </form>
  </div>
</template>
...
export default defineComponent({
  ...
  mounted: function () {
    ...
    if (this.messageList[this.messageList.length - 1].text !== this.helloWorld) {
